...
describe("DELETE /stages/:id", () => {
   ...
   it("should return 200 when stage is successfully deleted", async () => {
      ...
      // Verify stage was soft deleted (deletedAt is set)
      const stage = await prisma.stages.findUnique({
         where: { id: testStage.id },
      });
      expect(stage).not.toBeNull();
      expect(stage?.deletedAt).not.toBeNull();
      expect(stage?.deletedAt).toBeInstanceOf(Date);
   });
   ...
   it("should return 403 when user does not have permission to delete the stage", async () => {
      ...
      // Try to delete with otherUser (who doesn't have access to isolatedStage)
      const response = await app.handle(
